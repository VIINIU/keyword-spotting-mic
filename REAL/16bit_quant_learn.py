...
class WWS_SNN(nn.Module):
    ...
    # 🚨 초기화 함수 인자 수정
    def __init__(self, num_inputs, num_hiddens_1, num_hiddens_2, num_outputs, beta, threshold, spike_grad):
        super().__init__()
        
        # quantized_threshold = quantize_qmn(torch.tensor(threshold), self.QT_M, self.QT_N).item() # 나중에 활성화하기 양자화
        quantized_threshold = threshold # 🚨 그냥 원본 float threshold (0.5) 사용
        # 1. 입력층 -> 은닉층 1
    ...
    def forward(self, x, mem1, mem2, mem3):
        ...
        cur2 = self.fc2(spk1) # 🚨 2번째 층 통과
        spk2, mem2 = self.lif2(cur2, mem2) # 🚨 2번째 층 통과
        
        cur3 = self.fc3(spk2) # 🚨 3번째 층 (입력: spk2)
        spk3, mem3 = self.lif3(cur3, mem3) # 🚨 3번째 층
...
if __name__=="__main__":
    ...
    net = WWS_SNN(N_MELS, NUM_HIDDENS_1, NUM_HIDDENS_2, NUM_OUTPUTS, BETA, THRESHOLD, spike_grad).to(device)
    loss_fn = nn.CrossEntropyLoss(weight=class_weights).to(device) 
    optimizer = torch.optim.Adam(net.parameters(), lr=LEARNING_RATE, betas=(0.9, 0.999))
    
    print(f"SNN 학습 준비 완료. 총 배치 수: {len(train_loader)}")
    ...
    # === D. 학습 결과 저장 및 FPGA 변환용 추출 (유지) ===
    # 🚨 최종 저장 전, 가중치를 다시 한번 양자화하여 FPGA용으로 저장
    net.quantize_parameters()
    print("\n✅ 훈련 완료. 최종 FPGA용 가중치 양자화 적용 완료.")
    
    MODEL_SAVE_PATH = "./wws_snn_final_weights.pth"
    torch.save(net.state_dict(), MODEL_SAVE_PATH)
    print(f"\n✅ PyTorch 모델 가중치가 {MODEL_SAVE_PATH}에 저장되었습니다.")
    
    os.makedirs(FPGA_WEIGHTS_DIR, exist_ok=True)
    
    # FC1 (입력층 -> 은닉층)
    W1 = net.fc1.weight.data.numpy()
    B1 = net.fc1.bias.data.numpy()
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "W1.npy"), W1)
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "B1.npy"), B1)
    
    # FC2 (은닉층 -> 출력층)
    W2 = net.fc2.weight.data.numpy()
    B2 = net.fc2.bias.data.numpy()
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "W2.npy"), W2)
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "B2.npy"), B2)
    
    # 🚨 FC3 (은닉층2 -> 출력층) (추가!)
    W3 = net.fc3.weight.data.numpy()
    B3 = net.fc3.bias.data.numpy()
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "W3.npy"), W3)
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "B3.npy"), B3)
    
    # LIF 뉴런 하이퍼파라미터 (Q5.11 양자화된 THRESHOLD를 사용)
    quantized_threshold_val = quantize_qmn(torch.tensor(THRESHOLD), WWS_SNN.QT_M, WWS_SNN.QT_N).item()
    
    lif_params = {
        'BETA_VAL': BETA, 
        'THRESHOLD_VAL': quantized_threshold_val, # 🚨 양자화된 임계값 저장
        'QW_M': WWS_SNN.QW_M,
        'QW_N': WWS_SNN.QW_N,
        'QT_M': WWS_SNN.QT_M,
        'QT_N': WWS_SNN.QT_N,
    }
    np.save(os.path.join(FPGA_WEIGHTS_DIR, "LIF_params.npy"), lif_params)
    print("작동 완료")